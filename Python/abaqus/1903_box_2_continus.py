...
# 文件名参数
if angle1 == angle2:
	model_name = 'csb_' + str(angle1)
	job_name = 'csb_' + str(angle1) + '_out'
	res_out = './cae/csb_' + str(angle1) + '_res.json'
	rec_out = './cae/csb_' + str(angle1) + '_rec.json'
else:
...
for i in range(ns):
	...
	i3 = my_assembly.Instance(name = 'ISupport-' + str(i+2*ns+1), 
		part = support, dependent = ON)
	l3 = i3.vertices.findAt(((0, -0.2, 0.25), ))
	l_support[i+2*ns] = l3
	i_support[i+2*ns] = i3
	...
	i4 = my_assembly.Instance(name = 'ISupport-' + str(i+3*ns+1), 
		part = support, dependent = ON)
	l4 = i4.vertices.findAt(((0, -0.2, 0.25), ))
	l_support[i+3*ns] = l4
	i_support[i+3*ns] = i4
...
res_2 = get_path(0, 0, 40, variable2, 2)
