...
# 文件名参数
if angle1 == angle2:
	model_name = 'ssb_p_' + str(angle1)
	job_name = 'ssb_p_' + str(angle1) + '_out'
	res_out = './cae/ssb_p_' + str(angle1) + '_res.json'
	rec_out = './cae/ssb_p_' + str(angle1) + '_rec.json'
else:
	model_name = 'ssb_r_p_' + str(angle1) + '_' + str(angle2)
	job_name = 'ssb_r_p_' + str(angle1) + '_' + str(angle2) + '_out'
	res_out = './cae/ssb_r_p_' + str(angle1) + '_' + str(angle2) + '_res.json'
	rec_out = './cae/ssb_r_p_' + str(angle1) + '_' + str(angle2) + '_rec.json'
...
full_beam.BaseSolidExtrude(sketch = s1, depth = 100)

full_plate = box_1.Part(name = 'FullPlate', 
	dimensionality = THREE_D, type = DEFORMABLE_BODY)
full_plate.BaseSolidExtrude(sketch = s2, depth = 100)
...
my_assembly.translate(instanceList = ('ICutCube-5',), vector = (0, 0, length))
my_assembly.rotate(('ICutCube-5',), (0, 0, length), (0, 1, 0), angle2)

my_assembly.translate(instanceList = ('ICutCube-6',), 
	vector = (0, 0, length - thickness - 200))
my_assembly.rotate(('ICutCube-6',), (0, 0, length), (0, 1, 0), angle2)
...
# 合并横隔板和主梁
my_assembly.makeIndependent(instances = (i_cut_beam, i_plate_1, i_plate_2))
i_beam = my_assembly.InstanceFromBooleanMerge('Beam', 
	instances = (i_cut_beam, i_plate_1, i_plate_2), keepIntersections = True)
beam = box_1.parts['Beam']
my_assembly.makeIndependent(instances = (i_beam,))

# 建立支座
ns = len(support_loc)
l_support = range(2*ns)
i_support = range(2*ns)

for i in range(ns):
	i1 = my_assembly.Instance(name = 'ISupport-' + str(i+1), 
		part = support, dependent = ON)
	l1 = i1.vertices.findAt(((0, -0.2, 0.25), ))
	l_support[i] = l1
	i_support[i] = i1
	...
	i2 = my_assembly.Instance(name = 'ISupport-' + str(i+ns+1), 
		part = support, dependent = ON)
	l2 = i2.vertices.findAt(((0, -0.2, 0.25), ))
	l_support[i+ns] = l2
	i_support[i+ns] = i2
...
box_1.DisplacementBC(name='BC-2', createStepName='Initial', 
	region=(l_support[len(l_support)/2], ), u1=SET, u2=SET)
box_1.DisplacementBC(name='BC-3', createStepName='Initial', 
	region=tuple(l_support[1:len(l_support)/2]), u2=SET, u3=SET, localCsys=datum)
box_1.DisplacementBC(name='BC-4', createStepName='Initial', 
	region=tuple(l_support[len(l_support)/2+1:]), u2=SET)
...
p_force = i_beam.vertices.findAt(((-3.5, 0, 15 + 3.5 * tan(angle1*pi/180)), ))
...
box_1_odb = session.openOdb(name = job_name + '.odb', readOnly = False)
...
res_1 = get_path(-3.3, 0, 0, variable1, 1)
res_2 = get_path(0, 0, 20, variable2, 2)
